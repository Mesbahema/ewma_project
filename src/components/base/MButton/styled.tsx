...
import { MButtonStyleProps } from './types';
import { keyframes } from '@emotion/react';
import { themeObject } from '@/provider/AppThemeProvider';

function getPaletteColor({ palette, degree }: AppBaseColorType) {
    if (!palette) {
        return themeObject.palette.primary.main;
    }
    if (!degree || !themeObject.palette[palette] || !(degree in themeObject.palette[palette])) {
...
export const BSButton = styled.button(({ theme, disabled }) => {
    ...
    textDecoration: 'none',
    borderRadius: propTheme?.shape?.borderRadius?.common,
    padding: '8px 16px',
